...
case "CHANGE_STATUS": {
  ...
  return {
    ...state,
    // Store the previous state of tasks so we can undo once
    lastAction: {
      type: "CHANGE_STATUS",
    ...
    history: [
      ...state.history,
      createHistory(state, updatedTasks),
    ],
  };
}
...
case "UNDO": {
  ...
  return {
    ...
    history: [
      ...state.history,
      createHistory(state, restoredTasks),
    ],
  };
}
    ...
    case "LOAD_TASKS": {
      const mergedTasks = [
        ...
        ...action.payload.filter(
          apiTask => !state.tasks.some(task => task.id === apiTask.id)
        ),
      ];
...
function createHistory(state, tasks) {
  const elapsed =
    ...
    (state.working && state.startTime ? Date.now() - state.startTime : 0);
